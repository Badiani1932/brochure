...
| Data | Modifica |
| ------ | ---------- |
| 2026-03-16 | **evento-esterno — modali + cookie non funzionanti (regressione)**: il blocco `<script>` principale (i18n, modali, cookie, hamburger, form) aveva codice video/autoplay orfano incollato subito dopo la chiusura dell'IIFE i18n (`})();`), con una `}` pendente e riferimento a `forcePlay` non dichiarata. Questo causava un `SyntaxError` silenzioso che impediva l'esecuzione di tutti i listener (`.ext-card[data-modal]` click, `#cookieAccept` click). **Fix**: estratto il codice video in un IIFE separato in un nuovo tag `<script>`. Validato con `node --check` — tutti e 4 i blocchi OK. |
| 2026-03-16 | **Font — SuperGroteskA come font corpo unico**: standardizzato `SuperGroteskA` come primo font-family nel body (`badiani-common.css`) e in tutti gli override espliciti nei file B2B, eventi, evento-esterno. Pulsante "Invia Richiesta" nel modale B2B (`b2b-form__submit`) cambiato da `font-family:inherit` a `font-family:'SuperGroteskA',sans-serif`. I titoli (h1-h3, classi `__title`, `__name`) mantengono `SuperGroteskDigits / SuperGroteskB`. |
| 2026-03-16 | **Rimozione completa `font-style:italic`**: eliminato `font-style:italic` da 12 selettori in 4 file: `b2b/index.html` (coppette-hint, coni-hint, conigf-hint, accessori-hint, torte-hint ×2, torte-sizes-note, contenitori-hint), `eventi/index.html` (ev-corp-info__subtitle), `eventi/experience/index.html` (exp-concept__subtitle, exp-concept quote), `eventi/speciale-aziende/index.html` (tag pill). Nessun testo del sito è più in corsivo. |
| 2026-03-16 | **Swipe gesture — viewer immagini mobile**: aggiunto supporto swipe orizzontale (dito) su tutti e 3 i viewer a scorrimento foto. Logica uniforme: `touchstart` memorizza `X/Y`; `touchend` scatta se `dx > 50px` e `dx > dy` (evita conflitti con scroll verticale). Listener `passive:true` — nessuna interferenza con scroll del DOM. Applicato a: `#gelatoFullscreen` (B2B gusti/monoporzioni), `#lingottoCarousel` (B2B lingotto), `#imgLightbox` (eventi/evento-esterno Cool Box). |
| 2026-03-16 | **Eventi — sezione corporate: layout a colonne → impilato**: la griglia interna `.ev-corp-details` usava `grid-template-columns:1fr 1fr`, producendo due colonne di larghezza disuguale ("Cosa possiamo personalizzare" più largo di "Ideale per"). Cambiata in `grid-template-columns:1fr` (colonna unica). `border-right` su `.ev-corp-col--customize` sostituito con `border-bottom` + `padding-bottom`; aggiunto `padding-top` su `.ev-corp-col--ideal`. Rimosso il `@media(min-width:901px)` con `padding-left` extra (non più necessario). Regole mobile semplificati (la griglia già 1fr, rimossi gli override ridondanti). File: `eventi/index.html`. |
| 2026-03-16 | **B2B Monoporzioni — foto vetrina desktop**: limitata la dimensione della `.pezziduri-hero-overlay` su desktop (`>768px`): rimossi i margini negativi full-bleed, immagine centrata a dimensione naturale (`width:auto; max-height:70vh`). Mobile invariato (full-bleed, `height:auto`). |
| 2026-03-16 | **B2B Modale — padding gusti compromesso dopo tab Monoporzioni**: quando il modale veniva aperto dal pulsante Monoporzioni (che imposta `is-pezziduri-overlay`, azzerando il `padding`) e poi si cambiava tab su Gusti, il branch `isGelato` di `setActiveCategory` saltava `renderCategoryContent()` — unico punto dove le classi venivano rimosse. **Fix**: aggiunto `categoryContent.classList.remove('is-pezziduri', 'is-pezziduri-overlay')` all'inizio del branch `isGelato`. |
| 2026-03-16 | **magazine/ — pagina articoli vuota**: il blocco `<script>` principale (riga 453, ~570 righe) mancava del tag `</script>` di chiusura subito dopo la chiamata `})();` finale. Il browser interpretava tutto il `<footer>` e il resto dell'HTML come JavaScript, causando un errore di sintassi silenzioso che impediva l'esecuzione di `renderArticles()`. **Fix**: inserito `</script>` nella riga corretta. Tutti e 4 i blocchi script validano ora senza errori. |
| 2026-03-16 | **Cookie banner — non si chiude (mobile)**: rimossa `visibility:hidden` da `.cookie-banner.is-hidden` (si attivava istantaneamente, non essendo nella lista `transition`). Aggiunto `flex:none; width:100%` su `.cookie-banner__text` nel breakpoint mobile, dove `flex:1 1 400px` veniva interpretato come altezza minima di 400px in layout `flex-direction:column`. Fix in `assets/badiani-common.css`. |
| 2026-03-16 | **evento-esterno — modali non si aprivano + cookie non si chiudeva**: il blocco `<script>` principale aveva codice video/autoplay orfano fuori da qualsiasi funzione dopo la chiusura dell'IIFE i18n, con una `}` pendente e chiamata a `forcePlay` non dichiarata. Questo provocava un errore di sintassi silenzioso che impediva la registrazione dei listener per i click sui modali `.ext-card[data-modal]` e sul pulsante `#cookieAccept`. **Fix**: estratto il codice video in un nuovo IIFE separato (`forcePlay`) in un secondo tag `<script>`, identico al pattern in `b2b/index.html`. Validato con `node -e "new Function(...)"` — tutti i blocchi OK. |
